...
export default
class AtomCWLViewerView extends ScrollView {
  ...
  serialize () {
    return {
      deserializer: 'AtomCWLViewerView',
  ...
  handleEvents () {
    ...
    if (atom.config.get('atom-cwl-viewer.liveUpdate')) {
  ...
  updateImage () {
    ...
    var cwltool = atom.config.get('atom-cwl-viewer.cwltoolExecutable') || this.getDetectedPathFor('cwltool')
    var dot = atom.config.get('atom-cwl-viewer.graphvizDotExecutable') || this.getDetectedPathFor('dot')

    var cwltoolProcess    = childProcess.spawn(cwltool + ' --print-dot ' + this.editor.getPath() + ' | ' + dot + ' -Tsvg', [], {shell: true})
    var chunks = []
    cwltoolProcess.stdout.on('data', (chunk) => chunks.push(chunk))
    cwltoolProcess.stdout.on('end', () => {
  ...
  saveAs () {
    ...
    if (savePath) {
      ...
      var cwltool = atom.config.get('atom-cwl-viewer.cwltoolExecutable') || this.getDetectedPathFor('cwltool')
      var dot = atom.config.get('atom-cwl-viewer.graphvizDotExecutable') || this.getDetectedPathFor('dot')
      var cwltoolProcess    = childProcess.spawn(cwltool + ' --print-dot ' + this.editor.getPath() + ' | ' + dot + ' -T'+extension, [], {shell: true})
      cwltoolProcess.stdout.pipe(fileStream)
    }
  }
  ...
  copy () {
    var options = {
      ...
      dot: atom.config.get('atom-cwl-viewer.graphvizDotExecutable') || this.getDetectedPathFor('dot'),
      config: atom.config.get('atom-cwl-viewer.configFile'),
      charset: atom.config.get('atom-cwl-viewer.charset')
    }

    var cwltool = atom.config.get('atom-cwl-viewer.cwltoolExecutable') || this.getDetectedPathFor('cwltool')
    var dot = atom.config.get('atom-cwl-viewer.graphvizDotExecutable') || this.getDetectedPathFor('dot')

    var cwltoolProcess    = childProcess.spawn(cwltool + ' --print-dot ' + this.editor.getPath() + ' | ' + dot + ' -Tpng', [], {shell: true})
    var chunks = []
    cwltoolProcess.stdout.on('data', (chunk) => chunks.push(chunk))
    cwltoolProcess.stdout.on('end', () => {
  ...
  getTitle () {
    ...
    return 'Atom CWL View'
  }

  getURI () {
    return `atom-cwl-viewer://editor/${this.editorId}`
  }
  ...
  static content () {
    AtomCWLViewerView.div({
      class: 'atom-cwl-viewer native-key-bindings',
      tabindex: -1
    })
  }
}
